...
export const useTicketReports = () =>
  useQuery({ queryKey: ['ticket-reports'], queryFn: () => api.get('/itdesk/tickets/reports').then(r => r.data) });
...
export const useSLAPolicies = () =>
  useQuery({ queryKey: ['sla-policies'], queryFn: () => api.get('/itdesk/sla-policies').then(r => r.data) });
