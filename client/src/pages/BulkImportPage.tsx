      ...
      <PageBody width="wide">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            {/* Entity selector */}
            <Card padding="sm">
              <p className="block text-sm font-medium text-fg mb-2">Import as</p>
            ...
            <button
              ...
              className="w-full flex items-center gap-2 justify-center px-4 py-3 border-2 border-dashed border-line-strong rounded-card text-sm text-fg-muted hover:border-accent hover:text-accent transition-colors"
            >
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <Card padding="none">
              <div className="px-4 py-3 border-b border-line-subtle flex items-center justify-between">
                ...
                {csvText && <span className="text-xs text-fg-subtle">{csvText.split('\n').length - 1} rows</span>}
              </div>
              <div className="p-3">
                <Textarea
                    ...
                    {preview.rows.map((row, i) => (
                      <tr key={i}>
                        {Object.values(row).map((v, j) => (
                          <Td key={j} className="max-w-xs truncate">{v}</Td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
