...
import {
  RowActions, EmptyState, Badge, PageHeader, Button, IconButton, Modal, Card,
  Field, Input, Textarea, Spinner, invoiceStatusVariant,
                ...
                <div className="flex items-start justify-between flex-wrap gap-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-xs font-mono text-fg-subtle">{inv.invoiceNumber}</span>
                      <h3 className="font-semibold text-fg">{inv.title}</h3>
                      <Badge variant={invoiceStatusVariant[inv.status] ?? 'gray'}>{inv.status}</Badge>
                    </div>
                    {inv.deal?.title && <p className="text-xs text-fg-muted">Deal: {inv.deal.title}</p>}
                    {inv.quote?.title && <p className="text-xs text-fg-subtle">From quote: {inv.quote.title}</p>}
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-end pt-4 mt-3 border-t border-line-subtle">
            <div className="text-right text-sm space-y-0.5">
              ...
              <p className="text-lg font-bold text-fg">{money(grandTotal)}</p>
            </div>
          </div>
        </div>
      </Modal>
