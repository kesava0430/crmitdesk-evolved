...
export function AcceptInvitePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      await api.post('/auth/accept-invite', { token, name: form.name, password: form.password });
      setStatus('success');
      setTimeout(() => navigate('/login'), 2500);
    } catch (err: any) {
      setError(err?.response?.data?.message || 'Something went wrong. The invite may have expired.');
    } finally {
      setLoading(false);
    }
  }
