...
const ACTION_COLORS: Record<string, string> = {
  CREATE: 'bg-green-100 dark:bg-green-500/10 text-green-700 dark:text-green-400',
  UPDATE: 'bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400',
  DELETE: 'bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400',
  LOGIN: 'bg-purple-100 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400',
};
    ...
    <div className="p-6 max-w-6xl mx-auto animate-slide-up">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Track all actions across your organization</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap mb-4">
        <div className="relative flex-1">
          ...
          <input
            className="w-full pl-9 pr-4 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
        ...
        <select
          aria-label="Action"
          className="ui-input text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-500"
          value={action}
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Entity ID</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide hidden lg:table-cell">IP Address</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide hidden lg:table-cell">Browser</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {logs.map(log => (
                <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-semibold ${ACTION_COLORS[log.action] ?? 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300'}`}>
                      {log.action}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{log.entityType}</td>
                  <td className="px-4 py-3">
                    <code className="text-xs text-gray-500 dark:text-gray-400">{log.entityId?.slice(0, 12) ?? '—'}</code>
                  </td>
                  <td className="px-4 py-3 text-gray-400 dark:text-gray-500 text-xs hidden lg:table-cell">—</td>
                  <td className="px-4 py-3 text-gray-400 dark:text-gray-500 text-xs hidden lg:table-cell">—</td>
