...
import {
  ...
  DataTable, EmptyState, Spinner, type Column,
...
export default function AuditLogPage() {
  ...
  const columns: Column<AuditLog>[] = [
    ...
    { key: 'entityType', header: 'Type', muted: true, cell: log => log.entityType },
    {
      key: 'entityId',
      header: 'Entity ID',
      cell: log => <code className="text-[12px] text-fg-muted">{log.entityId?.slice(0, 12) ?? '—'}</code>,
    },
    {
      key: 'ip',
      header: 'IP Address',
        ...
        <Card padding="none">
          {isLoading ? (
            ...
            <DataTable
              ...
              rowKey={log => log.id}
              minWidth={640}
              empty={<EmptyState icon={<Shield />} title="No audit events found." />}
            />
          )}
        </Card>
