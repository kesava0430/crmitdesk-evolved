...
import {
  ...
  Field, Input, DataTable, EmptyState, Spinner, type Column,
...
export default function ApiKeysPage() {
  ...
  const columns: Column<ApiKey>[] = [
    { key: 'name', header: 'Name', cell: k => <span className="font-medium text-fg">{k.name}</span> },
    {
      key: 'prefix',
      header: 'Prefix',
      cell: k => <code className="bg-surface-sunken px-2 py-0.5 rounded-badge text-[12px] font-mono">{k.keyPrefix}…</code>,
    },
    {
      key: 'scopes',
      header: 'Scopes',
      ...
      <PageBody>
        ...
        {newKey && (
          ...
          >
            <div className="flex items-center gap-3 mt-2">
              <code className="flex-1 bg-surface border border-success/25 rounded-input px-3 py-2 text-[13px] font-mono text-fg truncate">
                {newKey}
              </code>
              <Button
                size="sm"
        ...
        <Card padding="none">
          <DataTable
            ...
            rowKey={k => k.id}
            minWidth={640}
            empty={<EmptyState icon={<Key />} title="No API keys yet. Create one to get started." />}
          />
        </Card>
      </PageBody>
