    ...
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
          {status?.connected && (
            <div className="space-y-4">
              {status.provider === 'GOOGLE_DRIVE' ? (
                ...
                <>
                  <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
                    <CheckCircle size={18} className="text-green-600 flex-shrink-0" />
                    <div>
                      <p className="text-sm font-medium text-green-800">Hosted storage connected</p>
                      <p className="text-sm text-green-700">Attachments are stored on our infrastructure.</p>
                    </div>
                  </div>
                  <div>
              ...
              {isOwner ? (
                ...
                <p className="text-xs text-gray-400">Only the org owner (Super Admin) can change storage settings.</p>
              )}
            </div>
          )}

          {/* Options to connect or switch — shown even when already connected, so switching provider doesn't require disconnecting first */}
          <div className={status?.connected ? 'pt-5 border-t border-gray-100' : ''}>
            {status?.connected && (
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div className="border border-gray-200 rounded-xl p-4">
                <p className="text-sm font-semibold text-gray-900 mb-1">Your own Google Drive</p>
                <p className="text-xs text-gray-500 mb-3">Free on every plan — no storage limit from us, since it's your own account.</p>
                {!status?.configured ? (
                  <div className="flex items-start gap-2 p-2.5 bg-amber-50 border border-amber-200 rounded-lg">
                    <AlertCircle size={14} className="text-amber-600 flex-shrink-0 mt-0.5" />
                    <p className="text-xs text-amber-700">Not set up on this deployment (missing GOOGLE_CLIENT_ID/SECRET).</p>
                  </div>
                ) : !isOwner ? (
          ...
          {!status?.connected && (
            <p className="text-xs text-gray-400">No storage is connected yet — uploading an attachment to any record will show an error until you connect one above.</p>
          )}
        </div>
      )}
    </div>
