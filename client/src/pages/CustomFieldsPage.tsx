...
import {
  ...
  PageHeader, PageBody, Card, Tabs, Button, Modal, Field, Input, Checkbox,
...
export default function CustomFieldsPage() {
  ...
  const columns: Column<CustomField>[] = [
    { key: 'label', header: 'Label', cell: f => <span className="font-medium text-fg">{f.label}</span> },
    {
      ...
      cell: f => <code className="text-xs bg-surface-sunken text-fg px-1.5 py-0.5 rounded">{f.fieldKey}</code>,
    },
    {
  ...
  return (
    <div className="animate-slide-up">
      ...
      <PageBody width="wide">
        <Card padding="none">
          {isLoading ? (
            <div className="py-12 text-center text-fg-muted">Loading…</div>
          ) : (
            <DataTable
              columns={columns}
              rows={fields}
