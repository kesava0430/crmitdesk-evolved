    ...
    <div className="p-6 max-w-4xl mx-auto animate-slide-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage customer access to the self-service support portal.</p>
        </div>
      ...
      <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 mb-6 flex items-center gap-4">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-violet-800 mb-0.5">Portal Link</p>
          <p className="text-xs text-violet-600 truncate">{portalUrl}</p>
        </div>
        <div className="flex gap-2 flex-shrink-0">
          <button onClick={copyPortalLink}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-white border border-violet-200 text-violet-700 rounded-lg hover:bg-violet-50">
      ...
      {isLoading ? <Spinner label="Loading…" /> : portalUsers.length === 0 ? (
        <div className="text-center py-16 bg-white border border-dashed border-gray-200 rounded-2xl">
          ...
          <h3 className="text-base font-semibold text-gray-700 mb-1">No portal customers yet</h3>
          <p className="text-sm text-gray-400 max-w-sm mx-auto mb-5">Add customers to give them access to submit and track support tickets without a full account.</p>
          <button onClick={() => setShowModal(true)}
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell">Last login</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {portalUsers.map(u => (
                <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${u.isActive ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {u.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-xs text-gray-500 hidden sm:table-cell">
                    {u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleDateString() : <span className="text-gray-300">Never</span>}
                  </td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-semibold text-gray-900">Add Portal Customer</h2>
              ...
              <div className="form-section">
                ...
                <div className="space-y-4">
                  ...
                  <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                    ...
                    <span className="text-gray-600">Send invite email with portal link</span>
                  </label>
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2 text-sm border border-gray-200 rounded-xl hover:bg-gray-50">Cancel</button>
