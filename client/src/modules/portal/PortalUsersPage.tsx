...
import {
  Spinner, RowActions, PageHeader, Button, Modal, Card, StatTile, Alert,
  Field, Input, Checkbox, Badge, Avatar, DataTable, EmptyState,
  type Column,
...
export function PortalUsersPage() {
  ...
  const columns: Column<PortalUser>[] = [
    ...
    },
    {
      ...
      hideBelow: 'sm',
      muted: true,
      cell: u => (u.lastLoginAt ? date(u.lastLoginAt) : <span className="text-fg-subtle">Never</span>),
    },
    {
      key: 'added',
      header: 'Added',
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5 animate-slide-up">
      <PageHeader
        title="Customer Portal"
        subtitle="Manage customer access to the self-service support portal."
        actions={<Button icon={<Plus size={15} />} onClick={() => setShowModal(true)}>Add Customer</Button>}
      />
