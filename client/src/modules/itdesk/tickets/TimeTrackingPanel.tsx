    ...
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-gray-800 text-sm">Time Tracking</span>
          {totalMinutes > 0 && (
        ...
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1 bg-brand-50 text-brand-700 rounded-lg text-xs font-medium hover:bg-brand-100"
        >
          ...
          <div className="flex items-end gap-3">
            <div className="flex-shrink-0">
              ...
              <input
                ...
                type="number"
                min="1"
                className="w-20 border border-gray-200 rounded-lg px-2 py-1.5 text-sm text-center focus:outline-none focus:ring-2 focus:ring-brand-500"
                placeholder="30"
                value={form.minutes}
      ...
      <div className="divide-y divide-gray-50">
        {entries.length === 0 ? (
          <div className="px-4 py-6 text-center text-sm text-gray-400">No time logged yet.</div>
        ) : (
          entries.map(e => (
            <div key={e.id} className="px-4 py-3 flex items-start justify-between group hover:bg-gray-50">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-gray-900">{formatTime(e.minutes)}</span>
                    <span className="text-xs text-gray-400">by {e.user.name}</span>
                  </div>
                  {e.description && <p className="text-xs text-gray-500 mt-0.5">{e.description}</p>}
                  <p className="text-xs text-gray-300 mt-0.5">{new Date(e.loggedAt || e.createdAt).toLocaleString()}</p>
                </div>
              </div>
              <button
