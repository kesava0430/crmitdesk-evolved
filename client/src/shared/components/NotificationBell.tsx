      ...
      {toasts.map(t => {
        ...
        return (
          <div key={t.id}
            ...
            <button onClick={() => onDismiss(t.id)} className="text-fg-subtle hover:text-gray-500 dark:hover:text-gray-400 flex-shrink-0">
              <X size={13} />
            </button>
          </div>
        );
              ...
              <div className="flex items-center gap-3">
                {push.isSupported && (
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-fg-subtle hover:text-gray-600 dark:hover:text-gray-300 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  >
              ...
              {notifications.length === 0 ? (
                ...
                notifications.slice(0, 20).map(n => (
                  <button key={n.id} onClick={() => markRead(n.id)}
                    className={`w-full text-left flex items-start gap-3 px-4 py-3 hover:bg-surface-hover transition-colors ${!n.readAt ? 'bg-brand-50/50 dark:bg-brand-500/10' : ''}`}>
