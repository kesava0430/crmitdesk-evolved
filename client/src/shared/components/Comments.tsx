  ...
  return (
    <div className="border-t pt-4 mt-4">
      ...
      <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
        {comments.length === 0 && (
          <p className="text-xs text-gray-400 text-center py-4">Nothing yet. Be the first to add a note.</p>
        )}
        {comments.map((c: any) => (
          <div key={c.id} className="flex gap-2 group">
            <div className="w-7 h-7 rounded-full bg-brand-100 text-brand-600 flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
            ...
            <div className="flex-1 bg-gray-50 rounded-xl px-3 py-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {c.author?.id === user?.id && (
                    <button onClick={() => remove.mutate(c.id)} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-300 hover:text-red-500 transition-all">
                      <Trash2 size={11} />
                    </button>
                  )}
                </div>
      ...
      {showReplyTemplates && (
        <div className="mb-2 ml-9 flex items-center gap-2">
          <LayoutTemplate size={13} className="text-gray-400 flex-shrink-0" />
      ...
      <div className="flex gap-2">
        <div className="w-7 h-7 rounded-full bg-brand-100 text-brand-600 flex items-center justify-center text-xs font-bold flex-shrink-0 mt-1">
      ...
      <p className="text-xs text-gray-400 mt-1 ml-9">Ctrl+Enter to send</p>
    </div>
  );
}
