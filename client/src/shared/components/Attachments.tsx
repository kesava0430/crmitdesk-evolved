...
export function Attachments({ entityType, entityId }: AttachmentsProps) {
  ...
  return (
    <div className="border-t pt-4 mt-4">
      ...
      {error && <div className="mb-2 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-2 py-1.5">{error}</div>}

      <div className="space-y-2 mb-3 max-h-64 overflow-y-auto">
        {isLoading ? (
          <p className="text-xs text-gray-400 text-center py-3">Loading…</p>
        ) : attachments.length === 0 ? (
          <p className="text-xs text-gray-400 text-center py-3">No files attached yet.</p>
        ) : (
          attachments.map((a: any) => {
            const Icon = iconFor(a.mimeType);
            const canDelete = a.uploader?.id === user?.id || user?.role === 'SUPER_ADMIN';
            return (
              <div key={a.id} className="flex items-center gap-2 group bg-gray-50 rounded-xl px-3 py-2">
                ...
                <button onClick={() => download(a)} className="p-1 text-gray-400 hover:text-brand-600 transition-colors flex-shrink-0" title="Download">
                  <Download size={14} />
                </button>
                {canDelete && (
                  <button onClick={() => remove.mutate(a.id)} className="p-1 opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 transition-all flex-shrink-0" title="Delete">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
      ...
      >
        ...
        <span className="text-xs text-gray-500">{upload.isPending ? 'Uploading…' : 'Click or drag a file to attach (max 25MB)'}</span>
        <input
          ref={fileInputRef}
          type="file"
          className="hidden"
