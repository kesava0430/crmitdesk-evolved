/**
 * Comprehensive seed script for CRMITDesk
 *
 * Run:  cd server && npm run db:seed
 *
 * Re-runnable: deletes the existing "techcorp" org (cascade) and re-creates everything.
 */

import { PrismaClient, UserRole, LeadStatus, DealStatus, TicketPriority, TicketStatus, ArticleStatus, ActivityType } from '@prisma/client';
...
async function main() {
  ...
  const existing = await prisma.organization.findUnique({ where: { slug: 'techcorp' } });
  if (existing) {
    console.log('Existing org found - deleting and re-seeding...');
    await prisma.organization.delete({ where: { id: existing.id } });
    await prisma.user.deleteMany({ where: { email: { in: ['admin@crmitdesk.com','crm@crmitdesk.com','sales@crmitdesk.com','itmanager@crmitdesk.com','itagent@crmitdesk.com'] } } });
    console.log('Cleared.');
  }
