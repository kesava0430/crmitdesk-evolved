...
import {
  ...
  EmptyState, SearchableSelect, Field, Input, Textarea, Alert, SkeletonStats, SkeletonTable,
  approvalStatusVariant,
} from '../../shared/components';
import { CalendarCheck, Plus, X, Check, Clock3, CheckCircle2 } from 'lucide-react';
    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {(data || []).map(b => (
        <StatTile
          ...
          value={<span className="tabular-nums">{b.remaining}<span className="text-xs font-normal text-fg-subtle"> / {b.leaveType.annualQuota} left</span></span>}
        />
      ))}
    </div>
  );
